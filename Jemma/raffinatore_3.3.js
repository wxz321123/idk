...
inputFiles.forEach(files => {
    ...
    console.log(new Date().toLocaleString().replace(/-/g, '/') + " --- Istanze da gestire: " + istanze_rimaste + " istanze");
    ...
    do
    {
        ...
        console.log(new Date().toLocaleString().replace(/-/g, '/') + " --- Creazione spazio..");
        for (let i = 0; i < free_cpu_server.length - 1 && combo_zero < 100; i++) {
            let app_aggiunte,
                precisione_dinamico_rovesciamento = precisione.precisione_rovesciamento,
                added_per_cicle = 0;
            do
            {
                app_aggiunte = 0;
                const match = the_best_solution_without_count_condition(single_server_status_sum_up[free_cpu_server[i]], application_sum_up, precisione_dinamico_rovesciamento, free_cpu_server.slice(i + 1, free_cpu_server.length));
                if (match.id_app != -1) {
                    precisione_dinamico = precisione_dinamico * 0.9;
                    for (let j = 0; j < application_sum_up[match.id_app].on_server_istance.length; j++) {
                        ...
                        if (rest_list.includes(application_sum_up[match.id_app].on_server_istance[j].id_server)) { //anche altre condizioni
                            ...
                            spazio_creato++;
                            added_per_cicle++;
                            console.log(new Date().toLocaleString().replace(/-/g, '/') + ` --- Spostato dal server: ${application_sum_up[match.id_app].on_server_istance[j].id_server} nel server: ${free_cpu_server[i]} l'istanza numero: ${application_sum_up[match.id_app].on_server_istance[j].id_istance}`);
            ...
            ;
            console.log(new Date().toLocaleString().replace(/-/g, '/') + " --- Ciclo " + i + "-esimo app spostate " + added_per_cicle);
        ...
        if (spazio_creato > 0) {
            ...
            precisione.precisione_rovesciamento = precisione.precisione_rovesciamento * 5;
        }
        console.log(`${new Date().toLocaleString().replace(/-/g, '/')} --- App spostate per creare spazio: ${spazio_creato}`);
        console.log(new Date().toLocaleString().replace(/-/g, '/') + " --- Riempimento server..");
        ...
        ;
        if (total_add == 0) {
            precisione.precisione_iniettamento = precisione.precisione_iniettamento * 5;
        } else {
        ...
        let total_swiched = 0;
        let precisione_dinamico_shuffle = precisione.precisione_shuffle;
        do
        {
            times++;
            const v_pendence_cpu = vector_pendence_cpu(single_server_status_sum_up, -1);
            const app_whish = [];
            swiched = 0;
            console.log(new Date().toLocaleString().replace(/-/g, '/') + " --- Cerco accoppiamenti per smistamento equo");
            //0 in 1 out
            ...
            console.log(new Date().toLocaleString().replace(/-/g, '/') + " --- Inizio fase scambio per ottimizzazione spazio");
                            ...
                            if (single_server_status_sum_up[app_whish[i].id_server_out[0]].list_id_app[j] == i) {
                                ...
                                for (let k = 0; k < application_sum_up[i].on_server_istance.length; k++) {
                                    if (application_sum_up[i].on_server_istance[k].id_server == app_whish[i].id_server_out[0]) {
                                        ...
                                        console.log(new Date().toLocaleString().replace(/-/g, '/') + ` --- Spostato dal server: ${application_sum_up[i].on_server_istance[k].id_server} nel server: ${app_whish[i].id_server_in[0]} l'istanza numero: ${application_sum_up[i].on_server_istance[k].id_istance} --- Spostare finora ${total_swiched}`);
    ...
    ;
    console.log(new Date().toLocaleString().replace(/-/g, '/') + " --- Sono rimasti: " + istanze_rimaste + " istanze");
    if (files.tag == 'A') {
        logger.write('#\n');
    }
});
    ...
    for (let j = 0; j < vector_app.length; j++) {
        ...
        for (k = 0; k < app.on_server_istance.length; k++) {
            if (in_id_server_list.includes(app.on_server_istance[k].id_server)) {
                k = app.on_server_istance.length;
            }
        }
        if (k > app.on_server_istance.length) {
...
function app_rompiscatole(app_status_all) {
    const quanto_rompe = [];
    const res_vector = [];
    app_status_all.forEach(app => {
        quanto_rompe.push({
            id_app: app.id_app,
            n_vincoli: app.other_conflict.length + app.inverse_other_conflict.length
        });
    })
    quanto_rompe.sort((a, b) => {
        if (a.n_vincoli > b.n_vincoli) 
            return -1;
        if (a.n_vincoli < b.n_vincoli) 
            return 1;
        return;
    })
    quanto_rompe.forEach(el => {
        res_vector.push(el.id_app);
    });
    return res_vector;
}