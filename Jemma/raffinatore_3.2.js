...
inputFiles.forEach(files => {
    ...
    do
    {
        ...
        for (let i = 0; i < free_cpu_server.length - 1 && combo_zero < 100; i++) {
            ...
            {
                app_aggiunte = 0;
                const match = the_best_solution_without_count_condition(single_server_status_sum_up[free_cpu_server[i]], application_sum_up, precisione.precisione_rovesciamento, free_cpu_server.slice(i + 1, free_cpu_server.length));
                if (match.id_app != -1) {
        ...
        do
        {
            ...
            for (let i = 0; i < v_pendence_cpu.length - 1; i++) {
                ...
                const solution_in = the_best_solution_without_count_condition(single_server_status_sum_up[server_id], application_sum_up, precisione.precisione_shuffle, v_pendence_cpu.slice(i + 1, v_pendence_cpu.length));
